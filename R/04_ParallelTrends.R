...
lines(usep$turnout[usep$state!="Colorado"] ~ usep$year[usep$state!="Colorado"], col="gray", lwd=2, lty=4)
lines(usep$turnout[usep$state=="Colorado"&usep$year < 2013] ~ usep$year[usep$state=="Colorado"&usep$year < 2013], lwd=2)
lines(usep$turnout[usep$state!="Colorado"&usep$year < 2013] ~ usep$year[usep$state!="Colorado"&usep$year < 2013], lwd=2, lty=4)
abline(v=2013, lty=3)
arrows(x0=2012, y0=0.85, x1=2012.9, y1=0.85, length=0.08) 
text(x=2009.5, y=0.85, labels="VBM Adoption 03'")   
legend("topright", legend=c("Colorado", "North Carolina"), lty=c(1, 4))
dev.off()
