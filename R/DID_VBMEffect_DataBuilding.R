...
library(tidyverse)
library(magrittr)
co2012 <- read_dta("Colo 2012 voted.dta")
co2016 <- read_dta("Colo 2016 voted.dta")
sup_co2012 <- read_dta("Colo reg voters 2012 estimated race.dta")      # Estimated race
sup_co2016 <- read_dta("colo 2016 with method of voting 09_17_18.dta") # Estimated race
co2012 <- co2012 %>% distinct(voter_id, .keep_all=T)    # Drop duplicates [A LOT, needs check]
co2016 <- co2016 %>% distinct(voter_id, .keep_all=T)    # Drop duplicates [A LOT, needs check]
sup_co2012 <- sup_co2012 %>% rename(voter_id = voterid) %>%
              distinct(voter_id, .keep_all=T)           # Drop duplicates [A LOT, needs check]
...
co12_16 <- co2012_sl %>%
           ...
           dplyr::select(-c(voterd_2016, Merge, estrace.y, state1.y, white.y)) %>%
...
library(tidyverse)
library(magrittr)
stack_co <- read_csv("Stack_Colorado_2012_2016.csv")
stack_nc <- read_csv("Stack_NC_2012_2016.csv")
