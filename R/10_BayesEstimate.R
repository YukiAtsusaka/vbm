...
################################################################################################
# (2) VISUALIZE PRIOR, LIKELIHOOD, AND POSTERIOR
################################################################################################
set.seed(618)
...
pdf("Bayes.pdf",width=8, height=4.5)
par(mar=c(4,4,1,4))

plot(y, y_prior, type = "n", xlim = c(0, 0.105), ylim = c(0, y_max),
         ylab = "density", xlab="Total VBM Effect")

# PRIOR, LIKELIHOOD, POSTERIOR
lines(y, y_prior, col = pal[1], lty = 2, lwd = 4)
...
legend("topleft", col = pal, lty = c(2, 1, 2), lwd=c(2,2,3), 
       cex = 1, bty = "n",
